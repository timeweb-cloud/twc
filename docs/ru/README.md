...
3. [Представление информации](#представление-информации)
    1. [Формат вывода](#формат-вывода)
    2. [Фильтрация списков, опция --filter](#фильтрация-списков-опция---filter)
4. [Управление облачными серверами](#управление-облачными-серверами)
...
6. [Автодополнение команд](#автодополнение-команд)
7. [Использование twc в качестве SDK](#использование-twc-в-качестве-sdk)

# Установка

## Установка с помощью pip (рекомендованный способ)

0. Установите [Python 3](https://www.python.org/downloads/). Версия Python должна быть не ниже **3.7**.
1. Если вы используете Windows пакетный менеджер pip уже установлен. Откройте терминал и введите команду `pip --version`, если командная оболочка сообщает, что команда не найдена, [установите pip](https://pip.pypa.io/en/stable/installation/). Вы также можете использовать для установки виртуальное окружение Python.
...
Также вам могут пригодиться переменные окружения. Используйте их вместо того, чтобы писать опцию `-c` или `-p`. Также можт быть полезно при автоматизации через Shell-скрипты. См. раздел [Переменные окружения](#переменные-окружения) ниже.
