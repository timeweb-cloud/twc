...
```
pip install ./twc_cli-2.0.0.tar.gz
```
...
Обратите внимание, что локация сервера также будет зависеть от выбранного пресета. Вот так можно посмотреть все доступные пресеты для локации `ru-1` (Россия, Санкт-Петербург) с типом диска NVMe SSD:
...
> ВАЖНО: Обратите внимание, что при переустановке сервера с выбранным ПО будет также переустановлена ОС!
...
*Изменено в версии v2.0.0*: Удалена опция `--to-server`.
...
*Изменено в версии v2.0.0*: Команда `set-ptr` заменена на `set`.
