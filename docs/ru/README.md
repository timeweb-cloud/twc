...
8. [Управление объектным хранилищем](#управление-объектным-хранилищем)
...
11. [Автодополнение команд](#автодополнение-команд)
12. [Использование twc в качестве SDK](#использование-twc-в-качестве-sdk)
...
Полагайтесь на имена полей, которые содержатся в JSON/YAML, а не на имена столбцов в таблице. Для фильтра по локациям также можно использовать алиас `region`.
...
Пресет определяет основные параметры сервера, такие как RAM, объём системного диска, тип диска, количество vCPU и др. Получить список всех доступных пресетов можно следующей командой:

```
twc server list-presets
```

Обратите внимание, что локация сервера также будет зависеть от выбранного пресета. Вот так можно посмотреть все доступные пресеты для локации `ru-1` (Россия, Санкт-Петербург):

```
twc server list-presets -f location:ru-1
```

Другой обязательный параметр это образ. Timeweb Cloud предлагает готовые к работе образы популярных операционных систем, получить их список можно командой:
...
Пример создания сервера на основе пресета с готовым образом:

```
twc server create --name dev-1 --image ubuntu-22.04 --preset-id 2573 --ssh-key ~/.ssh/id_rsa.pub
```

Создание сервера с произвольной конфигурацией пока возможно только в локации `ru-1` (Санкт-Петербург). Необходимо указать как минимум объём RAM, объём диска и число vCPU: 
...
Все операции требуют перезагрузки виртуальной машины для применения новых параметров.
...
Можно отслеживать прогресс создания образа командой (`Ctrl`+`C` для выхода):

```
watch echo Progress: $(twc image get e1048578-6365-44cd-9c6c-f663aa2866d0 -o json | jq .image.progress)%
```
...
После нажатия Enter twc предложит ввести пароль для базы данных. Вместо интерактивного ввода вы можете использовать опцию `--paassword` и ввести пароль прямо в командной строке. Учтите, что это может быть небезопасно.
...
> ВАЖНО: twc не реализует методы S3 API, поэтому для работы с объектами внутри бакетов используйте готовые S3-клиенты такие как [s3cmd](https://s3tools.org/s3cmd) или [rclone](https://rclone.org/), cм. [Получение конфигурации для S3-клиентов](#получение-конфигурации-для-s3-клиентов) ниже.
...
Чтобы создать бакет через twc сперва узнайте ID пресета, список пресетов можно получить командой:
...
```
twc s3 mb --preser-id 1234 имя-бакета
```

По умолчанию бакет будет приватный, вы можете задать опцию `--type public`, чтобы сделать его публичным.

> Обратите внимание, что при создании бакетов через twc (и API Timeweb Cloud) к имени бакета будет добавлен префикс. Имя созданного бакета будет отображено сразу по выполнении команды. Префикс не добавляется при создании бакетов через S3-клиенты.
...
В списке работает фильтрация через опцию `--filter`.
...
```
twc s3 set --type private имя-бакета
```
...
Если ваша командная оболочка не поддерживает операторы `>`, `>>` для перенаправления вывода, то вы можете воспользоваться опцией `--save-to`, чтобы сохранить конфигурацию в файл. Например (Windows cmd.exe):

```
twc s3 genconfig --client s3cmd --save-to %USERPROFILE%\.s3cfg
```

Осторожно, `--save-to` полностью перепишет файл, если он уже существует.
...
Переместить сервер `1234567` в другой `54321`:

```
twc project resource move --resource-id 1234567 --type server --to-project 54321
```
